<?php
  ...
  $n_dokter = query("SELECT * from dokter where id_dokter = $id")[0];
  ...
  if (isset($_POST['submit'])) {
    if (ubah($_POST) > 0) {
      ...
      echo "<script>alert('Data dokter gagal diubah');
      window.location.href='dokter.php'</script>";
      } 
  }
?>
                  ...
                  <form class="forms-sample" method="post" action="">
                    <div class="form-group row col-12">
                      <label for="nama_dokter" class="col-md-4 col-form-label" autocomplete="off">Nama Dokter</label>
                        <div class="col-md-8">
                          <input type="hidden" class="form-control" name="id_dokter" value="<?=$n_dokter["id_dokter"];?>">
                          <input type="text" class="form-control" name="nama_dokter" placeholder="Nama Dokter" value="<?=$n_dokter["nama_dokter"];?>" required>
                          <input type="submit" name="submit" value="Simpan" class="btn btn-primary rounded py-3 px-3 mt-4 mx-2">
                          <a href="dokter.php" class="btn btn-primary rounded py-3 px-3 mt-4">Cancel</a>
                        </div>
                    </div>
                  </form>
                </div>
