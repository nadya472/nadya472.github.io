<?php
  ...
  $nama_obat = query("SELECT * from obat where id_obat = $id")[0];
  
  if (isset($_POST['submit'])) {
    if (ubah3($_POST) > 0) {
      // pesan jika data tersimpan
      echo "<script>alert('Data Obat berhasil diubah'); 
      window.location.href='obat.php'</script>";
      } else {
      // pesan jika data gagal disimpan
      echo "<script>alert('Data obat gagal diubah');
      window.location.href='obat.php'</script>";
      } 
  }
?>
            ...
            <div class="content-wrapper">
                <div class="row">
                    ...
                    <div class="col-md-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Ubah Data Obat</h4>
                                <form class="forms-sample" method="post" action="">
                                    <div class="form-group row col-12">
                                        <label for="nama_obat" class="col-md-4 col-form-label" autocomplete="off">Nama Obat</label>
                                        <div class="col-md-8">
                                            <input type="hidden" class="form-control" name="id_obat" value="<?=$nama_obat["id_obat"];?>">
                                            <input type="text" class="form-control" name="nama_obat" placeholder="Nama Obat" value="<?=$nama_obat["nama_obat"];?>" required>
                                            <input type="submit" name="submit" value="Simpan" class="btn btn-primary rounded py-3 px-3 mt-4 mx-2">
                                            <a href="obat.php" class="btn btn-primary rounded py-3 px-3 mt-4">Cancel</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
